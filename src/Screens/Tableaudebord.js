...
const DashboardScreen = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Regroupement par nom_classeur
      const groupedResults = res.data.declarations.reduce((acc, declaration) => {
        const { nom_classeur } = declaration;
        if (!acc[nom_classeur]) {
          acc[nom_classeur] = { total: 0, nom_classeur }; // Initialisation
        }
        acc[nom_classeur].total += 1; // Incrément du total
        return acc;
      }, {});
        ...
        <div className="content-header">
          <div className="container-fluid">
            <h5 className="p-2 mb-3 bg-dark text-white">
              <i className="ion-ios-speedometer-outline mr-2" /> Tableau de Bord des Déclarations
            </h5>
