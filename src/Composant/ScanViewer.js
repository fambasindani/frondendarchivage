...
const ScanViewer = () => {
  ...
  const handleScanDocument = async () => {
    try {
        ...
        Swal.fire({
          ...
          html: `
            <div style="text-align: center;">
              ...
              <p class="text-muted small">Le document sera automatiquement envoyé au serveur</p>
            </div>
          `,
          showConfirmButton: false,
          allowOutsideClick: false,
        ...
        const checkScanCompletion = setInterval(async () => {
          ...
          if (attempts >= maxAttempts) {
            ...
            Swal.fire({
              icon: 'warning',
              title: 'Timeout',
              text: 'Le scan prend plus de temps que prévu'
            });
            setScanning(false);
            return;
          }
          ...
          try {
            ...
            if (!statusResponse.data.isScanning) {
              ...
              Swal.fire({
                icon: 'success',
                title: 'Scan terminé!',
                text: 'Le document a été scanné et envoyé avec succès',
                timer: 2000,
                showConfirmButton: false
              });
      ...
      Swal.fire({
        ...
        text: error.response?.data?.message || 'Impossible de démarrer le scan'
      });
      setScanning(false);
    }
  };
  ...
  const handleDownloadScan = async (scan) => {
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Erreur',
        text: error.response?.data?.message || 'Impossible de télécharger le fichier'
      });
    }
  };
  ...
  const handleDeleteScan = async (scan) => {
    const result = await Swal.fire({
      ...
      text: `Êtes-vous sûr de vouloir supprimer "${scan.name}" ?`,
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          icon: 'success',
          title: 'Supprimé!',
          text: 'Le scan a été supprimé avec succès.',
          timer: 1500,
          showConfirmButton: false
        });
